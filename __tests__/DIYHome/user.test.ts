...
import { GET as getUserById, PUT as updateUser } from "@/app/api/old/DIYHomes/users/[id]/route";
...
describe("User Management API Tests", () => {
  ...
  beforeAll(async () => {
    await prisma.userDIYHomes.deleteMany({ where: { email: testEmail } });
  });
  ...
  it("should fetch a single user by ID", async () => {
    const req = new NextRequest(`http://localhost:3000/api/DIYHomes/users/${testUserId}`);
  ...
  // ✅ Ensure user limit  (Max 50)
  it("should prevent creating more than 50 users ", async () => {
    const usersToCreate = [];
    for (let i = 0; i < 50; i++) {
      usersToCreate.push(
        prisma.userDIYHomes.create({
          data: {
    ...
    const data = await res.json();
    expect(data.error).toContain("Group has reached the maximum limit of 50 users");
  });

  // ✅ Ensure fetching a non-existent user returns 404
  it("should return 404 for non-existent user", async () => {
    const req = new NextRequest("http://localhost:3000/api/DIYHomes/users/invalid-user-id");
